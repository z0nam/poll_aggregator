calculate_house_effect <- function(party_index){
  ...
  house_effect <- rbind(house_effect, house_effect_by_party) # 이러면 되는데 왜 파일을 append 하고 있었을까.. 아, 어쩔 수 없었구나,,, return을 한 df만 할 수 있어서.. 
  ...
  aggregated_result_to_return <- list(global_result, house_effect)
  
  return(aggregated_result_to_return)
}
