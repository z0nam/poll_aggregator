# 관측수가 2 이상일 때 Bayesian Inference
# 작성: 최은철, 수정: 조남운

localdata = data %>% filter(area3==region) ## 지역 선택
...
## Initialization by parties

MU_START_SAE = localdata %>% group_by(enddate) %>% summarise(a = mean(SAE_raw))
MU_START_SAE <- MU_START_SAE$a[1] # 새누리당 초기값

MU_START_MIN = localdata %>% group_by(enddate) %>% summarise(a = mean(MIN_raw))
MU_START_MIN <- MU_START_MIN$a[1] # 민주당 초기값

MU_START_PEOPLE = localdata %>% group_by(enddate) %>% summarise(a = mean(PEOPLE_raw))
MU_START_PEOPLE <- MU_START_PEOPLE$a[1] # 국민의당 초기값

MU_START_JUS = localdata %>% group_by(enddate) %>% summarise(a = mean(JUS_raw))
MU_START_JUS <- MU_START_JUS$a[1] # 정의당 초기값

MU_START_UNK1 = localdata %>% group_by(enddate) %>% summarise(a = mean(UNK1_raw))
MU_START_UNK1 <- MU_START_UNK1$a[1] # 무소속 1 초기값

MU_START_UNK2 = localdata %>% group_by(enddate) %>% summarise(a = mean(UNK2_raw))
MU_START_UNK2 <- MU_START_UNK2$a[1] # 무소속 2 초기값

MU_START_UNK3 = localdata %>% group_by(enddate) %>% summarise(a = mean(UNK3_raw))
MU_START_UNK3 <- MU_START_UNK3$a[1] # 무소속 3 초기값


# final values

MU_FINISH_SAE = localdata %>% group_by(enddate) %>% summarise(a = mean(SAE_raw))
MU_FINISH_SAE <- MU_FINISH_SAE$a[length(MU_FINISH_SAE$a)] # 민주당 마지막값

MU_FINISH_MIN = localdata %>% group_by(enddate) %>% summarise(a = mean(MIN_raw))
MU_FINISH_MIN <- MU_FINISH_MIN$a[length(MU_FINISH_MIN$a)] # 민주당 마지막 값

MU_FINISH_PEOPLE = localdata %>% group_by(enddate) %>% summarise(a = mean(PEOPLE_raw))
MU_FINISH_PEOPLE <- MU_FINISH_PEOPLE$a[length(MU_FINISH_PEOPLE$a)] # 국민의당 마지막값

MU_FINISH_JUS = localdata %>% group_by(enddate) %>% summarise(a = mean(JUS_raw))
MU_FINISH_JUS <- MU_FINISH_JUS$a[length(MU_FINISH_JUS$a)] # 정의당 마지막값

MU_FINISH_UNK1 = localdata %>% group_by(enddate) %>% summarise(a = mean(UNK1_raw))
MU_FINISH_UNK1 <- MU_FINISH_UNK1$a[length(MU_FINISH_UNK1$a)] # 무소속 1 마지막값

MU_FINISH_UNK2 = localdata %>% group_by(enddate) %>% summarise(a = mean(UNK2_raw))
MU_FINISH_UNK2 <- MU_FINISH_UNK2$a[length(MU_FINISH_UNK2$a)] # 무소속 2 마지막값

MU_FINISH_UNK3 = localdata %>% group_by(enddate) %>% summarise(a = mean(UNK3_raw))
...
plot_results <- function(stan_m){
  ...
  names(ex) <- 1:days_between_elections
...
# 지역당 데이터가 가장 많은게 15개 정도로 하우스 이펙트 계산 자체가 비효율적. 이는 가져올 것이라면 전국 급 데이터에서 가져오는 것이 낫다고 판단. 

ac_SAE <- localdata %>%
  mutate(MidDate = startdate + (enddate-startdate)/2, 
         MidDateNum = as.integer(MidDate - START_DATE),
         p = SAE/100,
         se_SAE = sqrt(p*(1-p)/N)*100)
...
ac_UNK1 <- localdata %>%
  mutate(MidDate = startdate + (enddate-startdate)/2, 
         MidDateNum = as.integer(MidDate - START_DATE),
         p = UNK1/100,
         se_UNK1 = sqrt(p*(1-p)/N)*100)

ac_UNK2 <- localdata %>%
  mutate(MidDate = startdate + (enddate-startdate)/2, 
         MidDateNum = as.integer(MidDate - START_DATE),
         p = UNK2/100,
         se_UNK2 = sqrt(p*(1-p)/N)*100)

ac_UNK3 <- localdata %>%
  mutate(MidDate = startdate + (enddate-startdate)/2, 
         MidDateNum = as.integer(MidDate - START_DATE),
         p = UNK3/100,
         se_UNK3 = sqrt(p*(1-p)/N)*100)
...
d2_PEOPLE <- list(
  ...
  n_days = days_between_elections,
  y_values = ac_PEOPLE$PEOPLE,
  y_days = ac_PEOPLE$MidDateNum,
  y_n = nrow(ac_PEOPLE),
  y_se = ac_PEOPLE$se_PEOPLE
) 
...
d2_UNK1 <- list(
  ...
  n_days = days_between_elections,
  y_values = ac_UNK1$UNK1,
  y_days = ac_UNK1$MidDateNum,
  y_n = nrow(ac_UNK1),
  y_se = ac_UNK1$se_UNK1
)
...
d2_UNK2 <- list(
  ...
  n_days = days_between_elections,
  y_values = ac_UNK2$UNK2,
  y_days = ac_UNK2$MidDateNum,
  y_n = nrow(ac_UNK2),
  y_se = ac_UNK2$se_UNK2
) 
...
d2_UNK3 <- list(
  ...
  n_days = days_between_elections,
  y_values = ac_UNK3$UNK3,
  y_days = ac_UNK3$MidDateNum,
  y_n = nrow(ac_UNK3),
  y_se = ac_UNK3$se_UNK3
) 
...
system.time({
  ...
  warning=function(e){
    stan_mod2_SAE <- NA
    cat("could not do inference --> omit SAE\n")
  })
  ...
  tryCatch({
    ...
    stan_mod2_MIN <- NA
    cat("could not do inference --> omit MIN\n")
  })
  ...
  tryCatch({
    ...
    stan_mod2_PEOPLE <- NA
    cat("could not do inference --> omit PEOPLE\n")
  })
  ...
  tryCatch({
    ...
    stan_mod2_JUS <- NA
    cat("could not do inference --> omit JUS\n")
  })
  ...
  tryCatch({
    ...
    stan_mod2_UNK1 <- NA
    cat("could not do inference --> omit UNK1\n")
  })  
  ...
  tryCatch({
    ...
    stan_mod2_UNK2 <- NA
    cat("could not do inference --> omit UNK2\n")
  })
  ...
  tryCatch({
    ...
    stan_mod2_UNK3 <- NA
    cat("could not do inference --> omit UNK3\n")
  })  
}) 

# plot_results(stan_mod2_SAE) +
#   geom_point(data = ac_SAE, aes(x = MidDate, y = SAE_raw)) +
#   ggtitle(sprintf("Voting intention for the Minju Party between the %d %s elections"),
...
colnames_for_result = c('날짜','지지율','하한','상한')

if (!is.na(stan_mod2_SAE)){
  ...
  colnames(SAE_Final_Results) = colnames_for_result ## 새누리당 지지율 시계열
}

if (!is.na(stan_mod2_MIN)){
  ...
  colnames(MIN_Final_Results) = colnames_for_result ## 민주당 지지율 시계열
}

if(!is.na(stan_mod2_PEOPLE)){
  ...
  colnames(PEOPLE_Final_Results) = colnames_for_result ## 국민의당 지지율 시계열
}

if(!is.na(stan_mod2_JUS)){
  ...
  colnames(JUS_Final_Results) = colnames_for_result ## 정의당 지지율 시계열
}

if(!is.na(stan_mod2_UNK1)){
  ...
  colnames(UNK1_Final_Results) = colnames_for_result ## 무소속 1 지지율 시계열
}

if(!is.na(stan_mod2_UNK2)){
  ...
  colnames(UNK2_Final_Results) = colnames_for_result ## 무소속2  지지율 시계열
}

if(!is.na(stan_mod2_UNK3)){
  ...
  colnames(UNK3_Final_Results) = colnames_for_result ## 무소속3 지지율 시계열
}
